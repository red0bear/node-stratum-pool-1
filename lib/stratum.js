...
var StratumClient = function(options){
   ...
   function handleMessage(message){
       ...
       //var x = message.params[0][0];
      // console.log("what it show: " + x);
       
        switch(message.params[0][0])
       {
            case 'version-rolling':                   
                    ...
                    case 'mining.configure':
                            sendJson({"error": null, "id": 1, "result": {"version-rolling": true, "version-rolling.mask": "18000000"}});
                        break;
    ...
    function handleSubmitv1(message){
        if (!_this.authorized){
            sendJson({
                id    : message.id,
                result: null,
